...
buildroot: optee-os
	...
	@(cd .. && python build/br-ext/scripts/make_def_config.py \
		...
		--br-defconfig build/br-ext/configs/iiun \
		$(DEFCONFIG_GDBSERVER) \
...
.PHONY: linux-patches
linux-patches:
	for file in $(ROOT)/build/linux-patches/* ; do \
		cd $(ROOT)/linux/ && git apply $$file || \
			git apply $$file --reverse --check && \
			echo "$$file already applied" ; \
	done

.PHONY: linux-common
linux-common: linux-defconfig linux-patches
