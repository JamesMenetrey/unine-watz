...
#define BENCHMARK_START(X)                                  \
    TEE_Time start_##X, end_##X, X;                         \
    TEE_GetREETime(&start_##X)                              \

#define BENCHMARK_STOP(X)                                   \
    do {                                                    \
        TEE_GetREETime(&end_##X);                           \
        TEE_TIME_NANOS_SUB(&end_##X, &start_##X, &X);       \
    } while(0)
...
void speedtest1_begin_test(int iTestNum, const char *zTestName, ...){
  DMSG("Starting test %ld", iTestNum);
#if 0
...
int speedtest1_main(int argc, char **argv, char* benchmark_buffer, int benchmark_buffer_size){
...
#else
          EMSG("Canot operate on files in TrustZone.");
          TEE_Panic(TEE_ERROR_GENERIC);
#endif /* IN_TRUSTZONE */
        }
#endif
  ...
  snprintf(benchmark_buffer, benchmark_buffer_size, "100,%ld\n110,%ld\n120,%ld\n130,%ld\n140,%ld\n142,%ld\n145,%ld\n150,%ld\n160,%ld\n161,%ld\n170,%ld\n180,%ld\n190,%ld\n210,%ld\n230,%ld\n240,%ld\n250,%ld\n260,%ld\n270,%ld\n280,%ld\n290,%ld\n300,%ld\n310,%ld\n320,%ld\n400,%ld\n410,%ld\n500,%ld\n510,%ld\n520,%ld\n980,%ld\n990,%ld", \
            teetime_to_micro(exp100), \
            teetime_to_micro(exp110), \
            teetime_to_micro(exp120), \
            teetime_to_micro(exp130), \
            teetime_to_micro(exp140), \
            teetime_to_micro(exp142), \
            teetime_to_micro(exp145), \
            teetime_to_micro(exp150), \
            teetime_to_micro(exp160), \
            teetime_to_micro(exp161), \
            teetime_to_micro(exp170), \
            teetime_to_micro(exp180), \
            teetime_to_micro(exp190), \
            teetime_to_micro(exp210), \
            teetime_to_micro(exp230), \
            teetime_to_micro(exp240), \
            teetime_to_micro(exp250), \
            teetime_to_micro(exp260), \
            teetime_to_micro(exp270), \
            teetime_to_micro(exp280), \
            teetime_to_micro(exp290), \
            teetime_to_micro(exp300), \
            teetime_to_micro(exp310), \
            teetime_to_micro(exp320), \
            teetime_to_micro(exp400), \
            teetime_to_micro(exp410), \
            teetime_to_micro(exp500), \
            teetime_to_micro(exp510), \
            teetime_to_micro(exp520), \
            teetime_to_micro(exp980), \
            teetime_to_micro(exp990));
...
int bm_main(char* benchmark_buffer, int benchmark_buffer_size)
{
  // Configure SQLite to use a big buffer of memory for its heap
  int res;
  int size = 19 * 1024 * 1024;
  void* database_memory = TEE_Malloc(size, 0);

  int status = sqlite3_config(SQLITE_CONFIG_HEAP, database_memory, size, 2);
  if (status)
  {
      EMSG("Can't configure the heap buffer of SQLite. status: %d\n", status);
      return 1;
  }
  ...
  res = speedtest1_main(speedtest1_argc, speedtest1_argv, benchmark_buffer, benchmark_buffer_size);
